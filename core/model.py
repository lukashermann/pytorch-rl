...
class A3CCnnMjcModel2(Model):
    def __init__(self, args):
        super(A3CCnnMjcModel2, self).__init__(args)
        # build model
        # 0. feature layers
        # 84x84
        self.conv1 = nn.Conv2d(self.input_dims[0], 16, kernel_size=8, stride=4)
        self.rl1   = nn.ReLU()
        ...
        if self.enable_lstm:
            ...
            self.lstm_v  = nn.LSTMCell(7*7*32, self.hidden_dim, 1)
